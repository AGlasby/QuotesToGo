...
class QuoteViewController: UIViewController, UITextViewDelegate, NSLayoutManagerDelegate {
    ...
    @IBAction func showOptions(sender: AnyObject) {
        ...
        quoteMenu.frame = CGRectMake(0, -300, self.view.bounds.size.width, 300)
        optionsMenu = quoteMenu

        quoteMenu.quoteImageButton.addTarget(self, action: #selector(QuoteViewController.createQuoteImage), forControlEvents: .TouchUpInside)
        quoteMenu.shareQuoteButton.addTarget(self, action: #selector(QuoteViewController.shareQuote), forControlEvents: .TouchUpInside)
        quoteMenu.deleteQuoteButton.addTarget(self, action: #selector(QuoteViewController.deleteQuote), forControlEvents: .TouchUpInside)

        self.view.addSubview(quoteMenu)
        self.smallBookmarkTopConstraint.constant = 254
        UIView.animateWithDuration(0.6, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.8, options: .CurveEaseInOut, animations: { 
            ...
            let gestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(QuoteViewController.hideOnTap(_:)))
    ...
    func createQuoteImage() {
        self.performSegueWithIdentifier("createQuoteImage", sender: self)
